// Generated by BUCKLESCRIPT, PLEASE EDIT WITH CARE
'use strict';

var Atom = require("atom");
var Curry = require("bs-platform/lib/js/curry.js");
var Json_decode = require("@glennsl/bs-json/lib/js/src/Json_decode.bs.js");
var Pred$GclAtom = require("./View/Pred.bs.js");
var Decoder$GclAtom = require("./Decoder.bs.js");
var Caml_chrome_debugger = require("bs-platform/lib/js/caml_chrome_debugger.js");

var Specification = { };

function point(json) {
  return new Atom.Point(Json_decode.field("line", Json_decode.$$int, json) - 1 | 0, Json_decode.field("column", Json_decode.$$int, json) - 1 | 0);
}

var range = Decoder$GclAtom.fields((function (param) {
        if (param === "Loc") {
          return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                        var x = Json_decode.field("start", point, json);
                        var y = Json_decode.field("end", point, json);
                        return new Atom.Range(new Atom.Point(x.row, x.column), new Atom.Point(y.row, y.column));
                      })]);
        } else {
          return /* TagOnly */Caml_chrome_debugger.variant("TagOnly", 1, [(function (param) {
                        return new Atom.Range(new Atom.Point(0, 0), new Atom.Point(0, 0));
                      })]);
        }
      }));

function proofObligation(param) {
  return Json_decode.map((function (param) {
                return /* ProofObligation */Caml_chrome_debugger.simpleVariant("ProofObligation", [
                          param[0],
                          param[1]
                        ]);
              }), (function (param) {
                return Json_decode.pair(Json_decode.$$int, Pred$GclAtom.decode, param);
              }), param);
}

function specification(param) {
  return Json_decode.map((function (param) {
                return /* Specification */Caml_chrome_debugger.simpleVariant("Specification", [
                          param[0],
                          param[1],
                          param[2]
                        ]);
              }), (function (param) {
                return Json_decode.tuple3((function (param) {
                              return Json_decode.optional(Pred$GclAtom.decode, param);
                            }), Pred$GclAtom.decode, range, param);
              }), param);
}

var syntaxError = Decoder$GclAtom.fields((function (tag) {
        switch (tag) {
          case "DigHole" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                            return /* DigHole */Caml_chrome_debugger.variant("DigHole", 3, [Curry._1(range, json)]);
                          })]);
          case "ExcessBound" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                            return /* ExcessBound */Caml_chrome_debugger.variant("ExcessBound", 2, [Curry._1(range, json)]);
                          })]);
          case "MissingAssertion" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                            return /* MissingAssertion */Caml_chrome_debugger.variant("MissingAssertion", 1, [Curry._1(range, json)]);
                          })]);
          case "MissingBound" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                            return /* MissingBound */Caml_chrome_debugger.variant("MissingBound", 0, [Curry._1(range, json)]);
                          })]);
          case "MissingPostcondition" :
              return /* TagOnly */Caml_chrome_debugger.variant("TagOnly", 1, [(function (param) {
                            return /* MissingPostcondition */0;
                          })]);
          case "Panic" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                            return /* Panic */Caml_chrome_debugger.variant("Panic", 4, [Json_decode.string(json)]);
                          })]);
          default:
            throw [
                  Json_decode.DecodeError,
                  "Unknown constructor: " + tag
                ];
        }
      }));

var response = Decoder$GclAtom.fields((function (tag) {
        switch (tag) {
          case "OK" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (param) {
                            return Json_decode.map((function (param) {
                                          return /* OK */Caml_chrome_debugger.variant("OK", 2, [
                                                    param[0],
                                                    param[1]
                                                  ]);
                                        }), (function (param) {
                                          return Json_decode.pair((function (param) {
                                                        return Json_decode.array(proofObligation, param);
                                                      }), (function (param) {
                                                        return Json_decode.array(specification, param);
                                                      }), param);
                                        }), param);
                          })]);
          case "ParseError" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (param) {
                            return Json_decode.map((function (a) {
                                          return /* ParseError */Caml_chrome_debugger.variant("ParseError", 0, [a]);
                                        }), (function (param) {
                                          return Json_decode.array((function (param) {
                                                        return Json_decode.pair(point, Json_decode.string, param);
                                                      }), param);
                                        }), param);
                          })]);
          case "SyntaxError" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                            return /* SyntaxError */Caml_chrome_debugger.variant("SyntaxError", 1, [Curry._1(syntaxError, json)]);
                          })]);
          default:
            throw [
                  Json_decode.DecodeError,
                  "Unknown constructor: " + tag
                ];
        }
      }));

var Decode = {
  point: point,
  range: range,
  proofObligation: proofObligation,
  specification: specification,
  syntaxError: syntaxError,
  response: response
};

var parse = response;

exports.Specification = Specification;
exports.Decode = Decode;
exports.parse = parse;
/* range Not a pure module */
