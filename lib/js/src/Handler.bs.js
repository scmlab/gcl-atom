...
function ejectLastStatement(instance) {
  instance[/* editor */0].getCursorBufferPosition();
  return Async$GclAtom.resolve(/* () */0);
}
...
function markSpec(spec, instance) {
  ...
  var pre = trim(Pred$GclAtom.toString(spec[1]));
  var post = trim(Pred$GclAtom.toString(spec[2]));
...
exports.ejectLastStatement = ejectLastStatement;
