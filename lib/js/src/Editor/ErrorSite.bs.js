        ...
        switch (tag) {
          case "Global" :
              return /* Contents */Caml_chrome_debugger.variant("Contents", 0, [(function (json) {
                            return /* Global */Caml_chrome_debugger.variant("Global", 0, [Curry._1(Syntax$GclAtom.Loc.decode, json)]);
                          })]);
          case "Local" :
...
function toLoc(site, specifications) {
  if (site.tag) {
    ...
    return Rebase.$$Option.mapOr((function (spec) {
                  return Syntax$GclAtom.Loc.translateBy(1, 0, 1, 0, Syntax$GclAtom.Loc.translate(loc, spec[/* loc */3]));
...
function toRange(site, specifications) {
  return Syntax$GclAtom.Loc.toRange(toLoc(site, specifications));
}

function toString(site) {
  if (site.tag) {
    return "at " + (Syntax$GclAtom.Loc.toString(site[0]) + (" in #" + String(site[1])));
  } else {
    return "at " + Syntax$GclAtom.Loc.toString(site[0]);
  }
}
