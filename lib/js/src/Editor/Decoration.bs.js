...
function mark(type_, class_, loc, editor) {
  var range = Syntax$GclAtom.Loc.toRange(loc);
...
function overlayError(loc, editor) {
  var length = editor.getTextInBufferRange(Syntax$GclAtom.Loc.toRange(loc)).length;
...
function markSpec(spec, editor) {
  var match = spec[/* loc */3];
  if (match) {
    var end_ = match[1];
    var start = match[0];
    var startLoc_001 = Syntax$GclAtom.Pos.translateBy(0, 2, start);
    var startLoc = /* Loc */Caml_chrome_debugger.simpleVariant("Loc", [
        start,
        startLoc_001
      ]);
    var endLoc_000 = Syntax$GclAtom.Pos.translateBy(0, -2, end_);
...
function markSite(site, specifications, editor) {
  var loc = ErrorSite$GclAtom.toLoc(site, specifications);
  return Rebase.$$Array.fromList(Js_list.flatten(/* :: */Caml_chrome_debugger.simpleVariant("::", [
