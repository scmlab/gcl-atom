...
function mark(type_, class_, loc, editor) {
  var range = Base2$GclAtom.Loc.toRange(loc);
...
function overlayError(loc, editor) {
  var length = editor.getTextInBufferRange(Base2$GclAtom.Loc.toRange(loc)).length;
...
function markSpec(editor, spec) {
  var match = spec.loc;
  if (match) {
    var end_ = match[1];
    var start = match[0];
    var startLoc_001 = GCL$Guacamole.Pos.translateBy(0, 2, start);
    var startLoc = /* Loc */Caml_chrome_debugger.simpleVariant("Loc", [
        start,
        startLoc_001
      ]);
    var endLoc_000 = GCL$Guacamole.Pos.translateBy(0, -2, end_);
    ...
    var pre = trim(Curry._1(GCL$Guacamole.Syntax.Pred.toString, spec.pre));
    var post = trim(Curry._1(GCL$Guacamole.Syntax.Pred.toString, spec.post));
    var isQQ = pre === post;
    return Belt_List.toArray(Js_list.flatten(/* :: */Caml_chrome_debugger.simpleVariant("::", [
...
function markSite(site, specifications, editor) {
  var loc = Curry._2(GCL$Guacamole.$$Response.$$Error.Site.toLoc, site, specifications);
  return Belt_List.toArray(Js_list.flatten(/* :: */Caml_chrome_debugger.simpleVariant("::", [
