...
function markSpec(editor, spec) {
  ...
  if (match) {
    var end_ = match[1];
    var start = match[0];
    var startLoc_001 = View$Guacamole.Pos.translateBy(0, 2, start);
    var startLoc = /* Loc */Caml_chrome_debugger.simpleVariant("Loc", [
        start,
        startLoc_001
      ]);
    var endLoc_000 = View$Guacamole.Pos.translateBy(0, -2, end_);
    ...
    var pre = trim(Syntax$GclAtom.Pred.toString(spec.pre));
    var post = trim(Syntax$GclAtom.Pred.toString(spec.post));
    var isQQ = pre === post;
