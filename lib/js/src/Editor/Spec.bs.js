...
function fromCursorPosition(state) {
  var cursor = state[/* editor */0].getCursorBufferPosition();
...
function getPayloadRange(spec, state) {
  ...
  var start = state[/* editor */0].getBuffer().rangeForRow(startingRow, true).start;
  var end_ = state[/* editor */0].getBuffer().rangeForRow(endingRow, true).end;
  return new Atom.Range(start, end_);
}

function getPayload(spec, state) {
  var innerRange = getPayloadRange(spec, state);
  return state[/* editor */0].getBuffer().getTextInRange(innerRange);
}
...
function resolve(i, state) {
  ...
  Rebase.$$Option.forEach((function (spec) {
          ...
          state[/* editor */0].getBuffer().delete(Base$GclAtom.Loc.toRange(spec[/* loc */3]));
          state[/* editor */0].getBuffer().insert(start, payload.trim());
          return /* () */0;
...
function insert(lineNo, expr, state) {
  ...
  state[/* editor */0].setCursorScreenPosition(point);
  state[/* editor */0].insertText(assertion);
  return /* () */0;
}
