...
var Connection2$AgdaMode = require("agda-mode/lib/js/src/Connection2.bs.js");
...
function wire(connection) {
  ...
  Curry._1(connection[/* process */1][/* emitter */1][/* on */3], (function (param) {
          if (param.tag) {
            return Curry._1(connection[/* emitter */2][/* emit */1], /* Error */Caml_chrome_debugger.variant("Error", 1, [/* ConnectionError */Caml_chrome_debugger.variant("ConnectionError", 2, [param[0]])]));
          } else {
            var data = param[0];
...
function make(param) {
  return $$Promise.tapOk($$Promise.mapOk($$Promise.map(Connection2$AgdaMode.PathSearch.run("gcl"), (function (param) {
                        if (param.tag) {
                          return /* Error */Caml_chrome_debugger.variant("Error", 1, [/* PathSearchError */Caml_chrome_debugger.variant("PathSearchError", 0, [param[0]])]);
                        } else {
                          return /* Ok */Caml_chrome_debugger.variant("Ok", 0, [param[0]]);
                        }
                      })), (function (path) {
                    var $$process = Connection2$AgdaMode.Process.make(path, /* array */[]);
...
function send(request, connection) {
  ...
  if (result.tag) {
    return $$Promise.resolved(/* Error */Caml_chrome_debugger.variant("Error", 1, [/* ConnectionError */Caml_chrome_debugger.variant("ConnectionError", 2, [result[0]])]));
  } else {
    return promise;
  }
}
