...
function markSite(site, instance) {
  if (site.tag) {
    ...
    Rebase.$$Option.forEach((function (spec) {
            var range$1 = range.translate(spec[/* range */4].start, spec[/* range */4].start).translate(new Atom.Point(1, 0), new Atom.Point(1, 0));
            overlayError(range$1, instance);
            return mark("line-number", "line-number-error", range$1, instance);
          }), Rebase.$$Array.get(specs, 0));
  } else {
    var range$1 = site[0];
    overlayError(range$1, instance);
    mark("line-number", "line-number-error", range$1, instance);
  }
  return Async$GclAtom.resolve(/* [] */0);
}
