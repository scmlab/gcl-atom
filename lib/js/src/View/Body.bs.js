...
function Body$ProofObligation(Props) {
  ...
  var os = payload[3];
  var match = Rebase.$$Array.get(os, Rebase.$$Array.length(os) - 1 | 0);
  var origin;
  if (match !== undefined) {
    var x = match;
    var loc = x[0];
    origin = React.createElement(Link$GclAtom.make, {
          loc: loc,
          children: toString(x)
        });
  } else {
    origin = React.createElement(React.Fragment, undefined);
  }
